...
submission_1_rb = Template(
"""#!/bin/bash
#SBATCH --job-name={{gpu_job_name}}
...
#SBATCH --mem={{mem}}MB
#SBATCH -t 48:00:00
#SBATCH -o {{cwd}}/{{dataset}}/{{gpu_job_name}}.outcutoff
#SBATCH -e {{cwd}}/{{dataset}}/{{gpu_job_name}}.errcutoff
...
#SBATCH -t 48:00:00
{{require_gpu}}
#SBATCH -o {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/{{cycle}}_merge_out.out
#SBATCH -e {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/{{cycle}}_merge_err.err

module purge
module load cuda/10.2.89  # for cg2all_cuda
conda activate Calvados_MDP
{{merge_commandline}}
{{convert_saxs_commandline}}
""")
...
python3 {{cwd}}/pulchra.py --cwd {{cwd}} --dataset {{dataset}} --name $name --cycle {{cycle}} --num_cpus {{num_cpus}} --pulchra /groups/sbinlab/fancao/pulchra
done
fi

python3 {{cwd}}/optimize.py --path2config {{path2config}}""")

submission_3_rb = Template(
"""#!/bin/bash
#SBATCH --job-name=opt_{{cycle}}
#SBATCH --partition=qgpu
#SBATCH --nodes=1
...
python3 -u {{cwd}}/optimize.py --path2config {{path2config}}
{{submit_next}}
""")
...
#PBS -N {{record}}_sim{{cycle}}
#PBS -l nodes=1:ppn={{requested_cpunum}}:thinnode
#PBS -l mem={{mem}}mb
{{sim_dependency}}
#PBS -o {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/{{cycle}}_out_sim_{{replicas_list4MD_idx}}
#PBS -e {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/{{cycle}}_err_sim_{{replicas_list4MD_idx}}
...
{{sim_dependency}}
#PBS -l mem={{mem}}gb
#PBS -o {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/{{cycle}}_out_sim
#PBS -e {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/{{cycle}}_err_sim
...
echo ${thishost}>{{cwd}}/{{dataset}}/{{record}}/{{cycle}}/thishost.txt
# ray start --head --num-cpus {{Threads}} --port={{port}} --redis-shard-ports={{redis_shard_ports}} --node-manager-port={{node_manager_port}} --object-manager-port={{object_manager_port}} --dashboard-port={{dashboard_port}} --metrics-export-port={{metrics_export_port}} --dashboard-agent-listen-port={{dashboard_agent_listen_port}} --dashboard-agent-grpc-port={{dashboard_agent_grpc_port}} --ray-client-server-port={{ray_client_server_port}} --min-worker-port={{min_worker_port}} --max-worker-port={{max_worker_port}} > {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/port.txt
ray start --head --num-cpus {{Threads}} --port={{port}} --redis-shard-ports={{redis_shard_ports}} --node-manager-port={{node_manager_port}} --object-manager-port={{object_manager_port}} --include-dashboard=false --metrics-export-port={{metrics_export_port}} --dashboard-agent-listen-port={{dashboard_agent_listen_port}} --dashboard-agent-grpc-port={{dashboard_agent_grpc_port}} --ray-client-server-port={{ray_client_server_port}} --min-worker-port={{min_worker_port}} --max-worker-port={{max_worker_port}} > {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/port.txt
ray_exit_code=$?
while [ "$ray_exit_code" != "0" ]
do
echo "retrying....." >> {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/retry.txt
ray start --head --num-cpus {{Threads}} --port={{port}} --redis-shard-ports={{redis_shard_ports}} --node-manager-port={{node_manager_port}} --object-manager-port={{object_manager_port}} --include-dashboard=false --metrics-export-port={{metrics_export_port}} --dashboard-agent-listen-port={{dashboard_agent_listen_port}} --dashboard-agent-grpc-port={{dashboard_agent_grpc_port}} --ray-client-server-port={{ray_client_server_port}} --min-worker-port={{min_worker_port}} --max-worker-port={{max_worker_port}} > {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/port.txt
ray_exit_code=$?
done

res=$(cat {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/port.txt)
leftstring="start --address='"
rightstring=":{{port}}"
res=${res#*${leftstring}}
ip_prefix=${res%${rightstring}*}

ip_head=$ip_prefix:{{port}}
jobnodes=`uniq -c ${PBS_NODEFILE} | awk -F. '{print $1 }' | awk '{print $2}' | paste -s -d " "`
echo ${jobnodes}>{{cwd}}/{{dataset}}/{{record}}/{{cycle}}/jobnodes.txt
...
do
pbsdsh -h ${node} -v {{cwd}}/startWorkerNode.sh $ip_head {{Threads}} &
sleep {{name_idx}}
done
...
cd {{cwd}} || exit 
echo $ip_head
python3 -u {{cwd}}/simulate_distributed.py --ip_head $ip_head --config {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/{{config_simerge_filename}}
""")

submission_6 = Template(
"""#!/bin/bash
#PBS -W group_list=ku_10001 -A ku_10001
#PBS -N {{record}}_merge_{{cycle}}
#PBS -l nodes={{requested_resource}}:{{node}}
#PBS -l mem={{mem}}gb
{{merge_dependency}}
#PBS -o {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/{{cycle}}_merge_out
#PBS -e {{cwd}}/{{dataset}}/{{record}}/{{cycle}}/{{cycle}}_merge_err
...
def create_parameters(cwd, dataset, cycle, initial_type, wei_sigma=False):
    if initial_type == "C1":
        residues = pd.read_csv(f'{cwd}/{dataset}/residues_pub.csv').set_index('one')
        residues["lambdas"] = residues["CALVADOS1"]
        residues.to_csv(f'{cwd}/{dataset}/residues_pub.csv')
    if initial_type == "C2":
        residues = pd.read_csv(f'{cwd}/{dataset}/residues_pub.csv').set_index('one')
        residues["lambdas"] = residues["CALVADOS2"]
        if wei_sigma:
            residues.sigmas = integratingWeightedSigma(cwd, dataset)
        residues.to_csv(f'{cwd}/{dataset}/residues_pub.csv')
    if initial_type == "C3":
        residues = pd.read_csv(f'{cwd}/{dataset}/residues_pub.csv').set_index('one')
        ...
        residues.to_csv(f'{cwd}/{dataset}/residues_pub.csv')
    if initial_type == "0.5":
        residues = pd.read_csv(f'{cwd}/{dataset}/residues_-1.csv').set_index('one')
    if initial_type == "Ran":
        residues = pd.read_csv(f'{cwd}/{dataset}/residues_-1.csv').set_index('one')
        residues["Ran"] = np.random.rand(20)
        residues.lambdas = residues["Ran"]
        residues.to_csv(f'{cwd}/{dataset}/residues_-1.csv')
    if initial_type == "C3_based":
        residues = pd.read_csv(f'{cwd}/{dataset}/residues_-1.csv').set_index('one')
        residues.lambdas = pd.read_csv(f'{cwd}/residues_pub.csv').set_index('one')["CALVADOS3"]
        if wei_sigma:
            residues.sigmas = integratingWeightedSigma(cwd, dataset)
        residues.to_csv(f'{cwd}/{dataset}/residues_-1.csv')

    if initial_type == "C2_based":
        residues = pd.read_csv(f'{cwd}/{dataset}/residues_-1.csv').set_index('one')
        residues.lambdas = pd.read_csv(f'{cwd}/residues_pub.csv').set_index('one')["CALVADOS2"]
        if wei_sigma:
            residues.sigmas = integratingWeightedSigma(cwd, dataset)
        residues.to_csv(f'{cwd}/{dataset}/residues_-1.csv')
...
def reweight(cwd, dataset, cycle,dp,df,proteinsPRE,proteinsRgs, multidomain_names, proc_PRE, log_path, lambda_oneMax, allow_negative, effective_frac):
    ...
    if lambda_oneMax:
        ...
        f_out_of_01 = lambda df: df.loc[df.lambdas == np.nan, 'lambdas'].index  # remove 0-minimum limit
...
def determineThreadsnode(use_newdata, batch_sys, cost):
    ...
    if node == 'sbinlab_ib2':
        exclude_node = "#SBATCH --exclude=node593"  # 580-595
    else:
        exclude_node = ""

    return Threads, node, exclude_node
...
def resetexpRgErr(proteinsRgs, ratio):
    proteinsRgs["expRgErr"] = ratio * proteinsRgs.expRg.values
    return proteinsRgs
...
def integratingWeightedSigma(cwd, dataset):
    ...
    wei_sigma = residues.loc[pool, "sigmas"].mean()
    return wei_sigma

def minmax(df):
    print("rescaling lambda.....")
    if len(set(df.lambdas))==1:  # initialization 0.5
        return df
    else:
        arr = df.lambdas.to_numpy()
        arr = (arr - np.min(arr)) / (np.max(arr)-np.min(arr))
        df_minmax = df.copy()
        df_minmax.lambdas = arr
        return df_minmax
...
@ray.remote(num_cpus=1)
def SAXSout_MPI(t, n, subframes, config):
    cwd = config["cwd"]
    dataset = config["dataset"]
    record = config["record"]
    cycle = config["cycle"]
    dro = 1.0
    r0_min_factor = 1.025  # r0/rm
    r0_max_factor = 1.025  # r0/rm
    r0_N = 1
    cstFactor = 0
    I0 = 1.0
    for i in range(subframes):
        t[i].save_pdb(f"{cwd}/{dataset}/{record}/{cycle}/allatom/AA_{i+subframes*n}.pdb")
        os.system(f"Pepsi-SAXS {cwd}/{dataset}/{record}/{cycle}/allatom/AA_{i+subframes*n}.pdb {cwd}/experimentalSAXS/bift_{record}.dat -o {cwd}/{dataset}/{record}/{cycle}/allatom/AA_{i+subframes*n}.out --dro {dro} --r0_min_factor {r0_min_factor} --r0_max_factor {r0_max_factor} --r0_N {r0_N} -cst --cstFactor {cstFactor} --I0 {I0} >> {cwd}/{dataset}/{record}/{cycle}/PepsiSAXS.out")

def extractSAXS(cwd, dataset, name, cycle, n_frames, out_name=""):
    # if not os.path.isfile(f"{cwd}/{dataset}/{name}/{cycle}/{name}_SAXS.pkl"):
    SAXSdata = pd.DataFrame(columns=['q','expSAXS','expSAXSErr','calSAXS'],dtype=object)
    for frame_idx in range(n_frames):
        q = []
        expSAXS = []
        expSAXSErr = []
        calSAXS = []
        with open(f"{cwd}/{dataset}/{name}/{cycle}/allatom/AA_{frame_idx}.out", 'r') as file:
            for i in file.readlines():
                record = i.strip()
                if record[0]!='#':
                    record = record.split()
                    q.append(float(record[0]))
                    expSAXS.append(float(record[1]))
                    expSAXSErr.append(float(record[2]))
                    calSAXS.append(float(record[-1]))  # take into account that SAXS data might have more than 3 columns; in this case, 3 is not the calculated SAXS data
        SAXSdata.loc[frame_idx] = dict(q=np.array(q), expSAXS=np.array(expSAXS), expSAXSErr=np.array(expSAXSErr), calSAXS=np.array(calSAXS))
    SAXSdata.to_pickle(f"{cwd}/{dataset}/{name}/{cycle}/{name}_SAXS.pkl")

def checknecessaryfiles(config):
    cwd, dataset, cycle = config["cwd"], config["dataset"], config["cycle"]
    allproteins = pd.read_pickle(f"{cwd}/{dataset}/allproteins.pkl")
    check_pass = False
    while not check_pass:
        cur = True
        for record in allproteins.index:
            if not os.path.isfile(f'{cwd}/{dataset}/{record}/{cycle}/ini_beads_CA.pdb'):
                print(f"{record}/{cycle}/ini_beads_CA.pdb does not exist, maybe it is still in the queue?")
                os.system("sleep 10")
                cur = False
                break
        check_pass = cur
