...
parser.add_argument('--config', nargs='?', default='config_sim.yaml', const='config_sim.yaml', type=str)
parser.add_argument('--CUDA_VISIBLE_DEVICES', nargs='?', default='0', const='0', type=str)
...
while incomplete:
    try:
        ray.init(num_cpus=1 if gpu else int(config_sim["Threads"]*len(config_sim["replicas_list4MD"])), include_dashboard=False)
    except Exception:
